...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<SignUpVerification> signUp(String phoneNumber, String verifyKey, String publicKey) async {
    ...
    final response = await callWithDioErrorResolver(
      () => _authApiDataSource.initializeLoginProcess(requestBody),
    );
  ...
  Future<UserTokens> verifyOTPSignUp(String url, String otpCode) async {
    ...
    final response = await callWithDioErrorResolver(
      () => _authApiDataSource.verifyLoginProcess(url, request),
      logpassApiErrorMapper: _loginVerificationErrorMapper,
    );
